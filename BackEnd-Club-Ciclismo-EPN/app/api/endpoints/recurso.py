    ...
    try:
        ...
        if files_gallery:
            for gallery_file in files_gallery:
                ...
                db_imagen = RecursoImagen(
                    imagen_url=gallery_url,
                    recurso_id=db_recurso.id_recurso
                )
                db.add(db_imagen)
            db.commit()
            db.refresh(db_recurso)
...
def listar_todos_los_recursos(db: Session = Depends(get_db), skip: int = 0, limit: int = 100):
    recursos = db.query(Recurso).options(
        joinedload(Recurso.imagenes_secundarias)
...
def obtener_recurso_por_id(
    ...
    recurso = db.query(Recurso).options(
        joinedload(Recurso.imagenes_secundarias)
    ).filter(Recurso.id_recurso == id_recurso).first()
    
    if not recurso:
        raise HTTPException(status_code=404, detail="Recurso no encontrado")

    # --- 2. CONSTRUIMOS LAS URLs COMPLETAS ---
    base_url = str(request.base_url).rstrip('/')
    if recurso.imagen_url:
        recurso.imagen_url = f"{base_url}{recurso.imagen_url}"
    
    for img in recurso.imagenes_secundarias:
        img.imagen_url = f"{base_url}{img.imagen_url}"
...
def borrar_recurso(id_recurso: int, db: Session = Depends(get_db)):
    # ... (Tu lógica de 'borrar_recurso' va aquí sin cambios) ...
    recurso = db.query(Recurso).options(
        joinedload(Recurso.imagenes_secundarias)
    ).filter(Recurso.id_recurso == id_recurso).first()
    if not recurso:
    ...
    try:
        if recurso.imagen_url:
            ...
            if os.path.exists(file_path):
                os.remove(file_path)
        for img in recurso.imagenes_secundarias:
            gallery_file_path = img.imagen_url.lstrip('/')
...
def actualizar_recurso(
    ...
    # ... (Tu lógica de 'actualizar_recurso' va aquí sin cambios) ...
    ...
    try:
        ...
        db.refresh(db_recurso)
        if files_gallery:
            print(f"Añadiendo {len(files_gallery)} nuevas imágenes de galería...")
            for gallery_file in files_gallery:
                gallery_url = save_upload_file(gallery_file)
                db_imagen = RecursoImagen(
                    imagen_url=gallery_url,
